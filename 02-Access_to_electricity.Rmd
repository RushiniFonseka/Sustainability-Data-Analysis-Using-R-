---
title: "02-Access to electricity (% of population)"
author: "PS/2020/215"
date: "2024-02-23"
...
variable_summary=function(country,indicator,DF,df,missingCount){
  ...
  if(indicator[i] == "EG.ELC.ACCS.ZS"){
    df <- data.frame(t(DF[i, 5:68]))
    print(paste("Summary of Access to electricity (% of population) in", country[i], ":"))
    print(df)
    print(summary(df))
    
    barplot(as.matrix(df), beside = TRUE, main = paste("Access to electricity in", country[i]), 
              xlab = "Year", ylab = "Value(% of population)", col ="lightblue")
    boxplot(df,main = paste("Access to electricity in", country[i]))
...
if (!indicator_found) {  # Check the flag to see if the indicator is not found
    print("There is no indicator code as EG.ELC.ACCS.ZS in the provided data.")
  }
